...
function StatCard({
  ...
  const trend =
    typeof config.trend === "function"
      ? config.trend(cardData.subtitle)
      : config.trend;
  ...
  return (
    ...
    >
      ...
      <p
        className={`absolute -bottom-4 -right-2 text-[80px] font-black ${config.textColor} opacity-[0.04] select-none leading-none`}
      >
        {index + 1}
      </p>
...
export function Stat({
  ...
  if (isLoading || !data) {
    return (
      <div
        className={`grid gap-5 ${columns === 2 ? "grid-cols-2" : "grid-cols-4"}`}
      >
        {[1, 2, 3, 4].map((i) => (
          <div
            key={i}
    ...
    >
      {cardKeys.map((key, index) => (
        <StatCard
          key={key}
          cardKey={key}
          data={statsData[key as keyof typeof statsData]}
          index={index}
        />
      ))}
    </div>
